...
public function showmaintain(Request $request){
    ...
    // $m3 = strtotime($m3);
    if($sl == '1w'){
        $devices = $devices->where('maintain_date','>=',$today)->where('maintain_date','<=',$w1->toDateString());
    }
    if($sl == '1m'){
         $devices = $devices->where('maintain_date','>=',$today)->where('maintain_date','<=',$m1->toDateString());
    }
    if($sl == '2m'){
        $devices = $devices->where('maintain_date','>=',$today)->where('maintain_date','<=',$m2);
    }
    if($sl == '3m'){
        $devices = $devices->where('maintain_date','>=',$today)->where ('maintain_date','<=',$m3);
    }
    $devices = $devices->paginate(10);
